...
function normalizeWsHostName(host: string): string {
  const trimmed = host.trim().replace(/^wss?:\/\//i, '');
  const hostWithOptionalPort = trimmed.split('/')[0] || '';
  const portSeparatorIndex = hostWithOptionalPort.indexOf(':');
  const normalized = portSeparatorIndex >= 0
    ? hostWithOptionalPort.slice(0, portSeparatorIndex)
    : hostWithOptionalPort;
  return normalized.trim().toLowerCase();
}

function resolvePrimaryWssAddress(validHosts: Array<{ host: string; wss_port?: unknown }>): string {
  const broadcastHost = validHosts.find((hostInfo) => normalizeWsHostName(hostInfo.host) === DEFAULT_BILIBILI_LIVE_WS_HOST);
  const broadcastPort = Number(broadcastHost?.wss_port);
  if (Number.isFinite(broadcastPort) && broadcastPort > 0) {
    return buildWssAddress(DEFAULT_BILIBILI_LIVE_WS_HOST, broadcastPort);
  }

  const fallbackHost = validHosts.find((hostInfo) => {
    const port = Number(hostInfo.wss_port);
    return Number.isFinite(port) && port > 0;
  });
  const fallbackPort = Number(fallbackHost?.wss_port);
  return buildWssAddress(
    DEFAULT_BILIBILI_LIVE_WS_HOST,
    Number.isFinite(fallbackPort) && fallbackPort > 0 ? fallbackPort : DEFAULT_BILIBILI_LIVE_WS_PORT
  );
}
