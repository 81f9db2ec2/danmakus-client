...
import { liveSessionOutbox } from './liveSessionOutbox';
import {
  cloneAuthState,
...
const desktopClientVersion = `desktop@${__APP_VERSION__}`;
...
class DanmakuService {
  ...
  public async initialize(localConfig?: Pick<LocalAppConfigDto, 'cookieCloudKey' | 'cookieCloudPassword' | 'cookieCloudHost' | 'cookieRefreshInterval' | 'capacityOverride'>): Promise<void> {
    ...
    if (this.client && this.lastInitializationSignature === signature) {
      ...
      await this.client.refreshAuthState();
    ...
    this.setupListeners();
    await this.client.refreshAuthState({ force: true });
    this.applyStatusSnapshot();
  ...
  public clearRuntimeError(expectedError?: string | null): void {
    ...
    this.client.clearRuntimeError(expectedError);
    this.applyStatusSnapshot();
  }
  ...
  private applyCoreConfigState(config: CoreControlConfigDto): void {
    ...
    this.state.coreConfig.excludedServerRoomUserIds = [...(config.excludedServerRoomUserIds ?? [])];
