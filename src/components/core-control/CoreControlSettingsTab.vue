    ...
    <TooltipProvider>
      ...
      <Card class="bg-card/60">
        ...
        <CardContent class="space-y-5">
          ...
          <div class="space-y-2.5">
            <div class="flex items-center gap-1.5">
              <label class="text-xs font-medium text-muted-foreground">不补充分配的主播 UID</label>
              <Tooltip>
                <TooltipTrigger as-child>
                  <Info class="h-3.5 w-3.5 text-muted-foreground cursor-help" />
                </TooltipTrigger>
                <TooltipContent class="max-w-xs">
                  <p>这里填的 UID 只会阻止本站给你补充分配这些主播，不影响你手动添加的录制主播</p>
                </TooltipContent>
              </Tooltip>
            </div>
            <textarea
              ...
              rows="4"
              class="flex min-h-[96px] w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm outline-none placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
              placeholder="每行一个 UID，也支持逗号或空格分隔"
              @change="assignExcludedServerRoomUserIds"
            />
            <p class="text-[11px] text-muted-foreground">仅影响本站补充分配，不影响账号录制主播</p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent class="space-y-3">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-end">
            <div class="flex-1 space-y-1">
              ...
              <Input v-model="newRecordingUid" type="number" min="1" placeholder="输入主播 UID"
          ...
          <div v-for="item in recordings" :key="item.channel.uId"
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="flex items-center gap-1 rounded-md border bg-background/60 px-2 py-1">
                ...
                <Switch :model-value="isRecordingLiveNotificationEnabled(item.channel.uId)"
